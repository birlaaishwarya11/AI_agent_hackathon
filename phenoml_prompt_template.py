"""
PhenoML Prompt Template for Healthcare Query Processing
This module contains the prompt template for processing healthcare queries
and generating structured appointment booking data.
"""

PHENOML_HEALTHCARE_PROMPT = """
...
def create_phenoml_prompt(patient_query: str, additional_context: str = "") -> str:
    """
    Create a formatted prompt for PhenoML processing.
    
    Args:
        patient_query (str): The patient's healthcare query
        additional_context (str): Optional additional context or patient information
        
    Returns:
        str: Formatted prompt ready for PhenoML API
    """
    try:
        return PHENOML_HEALTHCARE_PROMPT.format(
            patient_query=patient_query,
            additional_context=additional_context
        )
    except Exception as e:
        print(f"Error formatting prompt: {{e}}")
        print(f"Patient query: {{repr(patient_query)}}")
        print(f"Additional context: {{repr(additional_context)}}")
        raise

# Alternative shorter prompt for simpler use cases
PHENOML_SIMPLE_PROMPT = """
You are a healthcare AI assistant. Analyze the patient query and provide structured appointment booking data in JSON format.

Requirements:
1. Assess symptoms and severity
2. Determine appropriate care level (primary care, urgent care, emergency, specialist)
3. Identify relevant clinical guidelines
4. Assign priority level
5. Output valid JSON following the appointment booking schema
...
def create_simple_phenoml_prompt(patient_query: str) -> str:
    """
    Create a simplified prompt for basic healthcare query processing.
    
    Args:
        patient_query (str): The patient's healthcare query
        
    Returns:
        str: Simplified formatted prompt
    """
    return PHENOML_SIMPLE_PROMPT.format(patient_query=patient_query)